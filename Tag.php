							...
							if(!$db){
								...
								$query = "SELECT
										...
										FROM
											...
											TU.FNAME \"FNAME\",
											TU.LNAME \"LNAME\",
											TF.FOLLOWINGUSER \"FOLLOWER\",
											TF.FOLLOWED \"FOLLOWED\"
											FROM 
												(SELECT
													T.TAGNAME \"TAGNAME\",
													U.USERNAME \"UNAME\",
												...
												LEFT OUTER JOIN TAGFOLLOW TF
												ON (TU.TAGNAME = TF.TAGTOFOLLOW)        
											) \"TUTF\"

										WHERE TUTF.TAGNAME = '".$tagId."'";
								$statement = oci_parse($db,$query);
								oci_execute($statement);
								while($row = oci_fetch_array($statement, OCI_BOTH+OCI_RETURN_NULLS)){
									$theTagName = $row[0];
									$tagUserName = $row[1];
									$tagUserFirstName = $row[2];
									$tagUserLastName = $row[3];
									if($row[5] == "1") {
										if($_SESSION['userName'] == $row[4]){
											$alreadyFollowing = true;
										}
										array_push($followers,$row[4]);
									}
								}								
							}
						?>
						<table border="0" width="100%">
							<tr>
								<td id="ProfilePic" width="20%" valign="top">							
									<img src="../Images/Tag_DP.jpg" height="145" width="140"></img>									
								</td>
								<td id="OtherDetails" valign="top">
									<h2><?php echo $theTagName; ?></h2>	
									<label>
										Created by: <label style="font-size:20;font-weight:bold;">
											<a href="User.php?id=<?php echo $tagUserName; ?>" style="text-decoration:none;color:inherit;">
												<?php echo $tagUserFirstName."	".$tagUserLastName; ?>
											</a>
										</label>
									</label>
									
									<p><?php echo count($followers); ?> people follow this tag!</p>
								</td>
							...
							<tr >
								<td colspan=3">
									...
									<hr/>
									<br/>									
									<label style="font-size:20;color:#000000;border-top-style:solid;">Questions using <?php echo $theTagName; ?></label>
									<br/>
									<?php
										$db = oci_connect("pdhar", "PunyabrataDhar", "oracle.cise.ufl.edu/orcl");
										//Connection Problem
									...
									<div style="max-height:500px;overflow:auto;">
										<p>
											<label><?php echo $theTagName; ?> is used by <?php echo $quesCount; ?> question(s) so far</label>
										</p>
